...
const page = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const response = await api.post(`/user/update/${params.id}`,form,{
            headers: {
    ...
    const fetch = async () => {
        const userdata = await api.get(`/user/get/${params.id}`,{
            headers: {
                    Authorization: `Bearer ${token}`,
                },
            withCredentials: true
        ...
        setFormData({
            ...
            contactno: userdata.data.user.contactno,
            DOB: userdata.data.user.DOB,
            gender: userdata.data.user.gender,
            department: userdata.data.user.department,
            dateOfJoining: userdata.data.user.dateOfJoining,
            status: userdata.data.user.status
        })
        setProfile(userdata.data.user.picture)
    }
    ...
    return (
        ...
        <h1 className="mb-6 font-semibold text-xl md:text-2xl text-center">Edit Employee</h1>
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {file && (
                ...
                {(!file && profile) &&<div className="mt-4 md:mt-0 w-24 h-24">
                    <Image
                        width={96}
                        height={96}
                        className="rounded-full w-full h-full object-cover"
            ...
            <button
                type="submit"
                className="bg-yellow-600 hover:bg-yellow-700 px-6 py-2 rounded-md font-semibold text-md text-white"
            >
